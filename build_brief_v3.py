...
def patch_theme_hyperlink_colors(pptx_path):
    """LibreOffice renders hyperlinked runs using the theme hlink and folHlink
    colours regardless of any explicit run colour, which pulls in the default
    Office blue and purple. Force both to brand teal so links stay on brand."""
            ...
            for name in names:
                ...
                if name.startswith("ppt/theme/theme") and name.endswith(".xml"):
                    ...
                    text = re.sub(
                        ...
                        "<a:hlink><a:srgbClr val=\"01A296\"/></a:hlink>",
                        text,
                    )
                    text = re.sub(
                        r"<a:folHlink><a:srgbClr val=\"[0-9A-Fa-f]{6}\"/></a:folHlink>",
                        "<a:folHlink><a:srgbClr val=\"01A296\"/></a:folHlink>",
                        text,
                    )
...
def build(data, out_path):
    ...
    add_rect(s3, left_x, Inches(4.72), left_w, Inches(0.62), WHITE, line_colour=AMBER_D)
    add_hyperlink_text(s3, "Purchase the suggested product now to get started",
                        data["stripe_link"], left_x + Inches(0.2), Inches(4.85),
    ...
    cred_paras = [{"text": "•  " + c, "size": 10.5, "colour": OFF_WHITE, "font": SANS,
                   "space_after": 8} for c in data["credibility_points"]]
    add_multiline(s3, cred_paras, right_x + Inches(0.25), Inches(2.3), right_w - Inches(0.5),
                  Inches(1.65), line_spacing=1.1)

    add_rect(s3, right_x + Inches(0.25), Inches(4.0), right_w - Inches(0.5), Pt(1), TEAL)
    contact_paras = [
    ...
    add_multiline(s3, contact_paras, right_x + Inches(0.25), Inches(4.2), right_w - Inches(0.5),
                  Inches(1.2))

    prs.save(out_path)
    patch_theme_hyperlink_colors(out_path)
